...
import naive.log
import naive.core
from naive._function_flatten import flatten
...
# Algorithms.
def falsum_algorithm(vector_size: int = 1) -> typing.List[naive.core.SystemFunction]:
...
def truth_algorithm(vector_size: int = 1) -> typing.List[naive.core.SystemFunction]:
...
def negation_algorithm(v: typing.List[naive.core.SystemFunction]) -> typing.List[naive.core.SystemFunction]:
...
def conjunction_algorithm(
        v1: typing.List[naive.core.SystemFunction],
        v2: typing.List[naive.core.SystemFunction]) -> \
        typing.List[naive.core.SystemFunction]:
...
# Scope.
ba1_scope = naive.core.Scope(
    scope_key=_SCOPE_BA1, structure_key=naive.core._STRUCTURE_SCOPE, language_key=_LANGUAGE_BA1, base_key='ba1_language',
    utf8='ba1_language', latex=r'\text{ba1_language}', html='ba1_language', usascii='ba1_language')

# Language.
ba1_language = naive.core.Language(
    scope_key=_SCOPE_BA1, structure_key=naive.core._STRUCTURE_LANGUAGE, language_key=_LANGUAGE_BA1, base_key='ba1_language',
    utf8='ba1_language', latex=r'\text{ba1_language}', html='ba1_language', usascii='ba1_language')

# Domains.
b = naive.core.Domain(
    scope_key=_SCOPE_BA1, structure_key=naive.core._STRUCTURE_DOMAIN, language_key=_LANGUAGE_BA1, base_key='b',
    utf8='𝔹', latex=r'\mathbb{B}', html='&Bopf;', usascii='B')
b2 = naive.core.Domain(
    scope_key=_SCOPE_BA1, structure_key=naive.core._STRUCTURE_DOMAIN, language_key=_LANGUAGE_BA1, base_key='b2',
...
def get_bn_domain(n):
    ...
    if not isinstance(n, int):
        naive.log.error('n must be an int')
    elif n < 1:
        naive.log.error('n must be > 1')
    elif n == 1:
        return b
    elif n == 2:
        return b2
        ...
        structure_key = naive.core._STRUCTURE_DOMAIN
        ...
        if naive.core.Concept.check_concept_from_decomposed_key(scope_key=scope_key, structure_key=structure_key,
                                                          language_key=language_key, base_key=base_key):
            return naive.core.Concept.get_concept_from_decomposed_key(scope_key=scope_key, structure_key=naive.core.structure_key,
                                                                language_key=language_key, base_key=base_key)
        else:
            return naive.core.Domain(
...
# Functions.
truth = naive.core.SystemFunction(
    scope_key=_SCOPE_BA1, structure_key=naive.core._STRUCTURE_FUNCTION, language_key=_LANGUAGE_BA1, base_key='truth',
    codomain=b, category=naive.core.SystemFunction.SYSTEM_CONSTANT, algorithm=truth_algorithm,
    utf8='⊤', latex=r'\top', html='&top;', usascii='truth', tokens=['⊤', 'truth', 'true', 't', '1'],
    arity=0, python_value=True)
falsum = naive.core.SystemFunction(
    scope_key=_SCOPE_BA1, structure_key=naive.core._STRUCTURE_FUNCTION, language_key=_LANGUAGE_BA1, base_key='falsum',
    codomain=b, category=naive.core.SystemFunction.SYSTEM_CONSTANT, algorithm=falsum_algorithm(),
    utf8='⊥', latex=r'\bot', html='&perp;', usascii='falsum', tokens=['⊥', 'falsum', 'false', 'f', '0'],
    arity=0, python_value=False)
negation = naive.core.SystemFunction(
    scope_key=_SCOPE_BA1, structure_key=naive.core._STRUCTURE_FUNCTION, language_key=_LANGUAGE_BA1, base_key='negation',
    codomain=b, category=naive.core.SystemFunction.SYSTEM_UNARY_OPERATOR, algorithm=negation_algorithm,
    utf8='¬', latex=r'\lnot', html='&not;', usascii='not', tokens=['¬', 'not', 'lnot'],
    domain=b, arity=1)
conjunction = naive.core.SystemFunction(
    scope_key=_SCOPE_BA1, structure_key=naive.core._STRUCTURE_FUNCTION, language_key=_LANGUAGE_BA1, base_key='conjunction',
    codomain=b, category=naive.core.SystemFunction.SYSTEM_BINARY_OPERATOR, algorithm=conjunction_algorithm,
    utf8='∧', latex=r'\land', html='&and;', usascii='and', tokens=['∧', 'and', 'land'],
    domain=b, arity=2)
disjunction = naive.core.SystemFunction(
    scope_key=_SCOPE_BA1, structure_key=naive.core._STRUCTURE_FUNCTION, language_key=_LANGUAGE_BA1, base_key='disjunction',
    codomain=b, category=naive.core.SystemFunction.SYSTEM_BINARY_OPERATOR, algorithm=disjunction_algorithm,
...
def satisfaction_index(phi: naive.core.Formula, variables_list=None):
    ...
    **sat_i**
    ...
    naive.log.debug(arguments_number=arguments_number)
    for argument_index in range(0, arguments_number):
        ...
        if isinstance(argument, naive.core.Formula) and \
                argument.is_system_function_call and \
                argument.codomain == b:
            # This argument is a Boolean Formula.
            naive.log.debug('This argument is a Boolean Formula')
        ...
        elif isinstance(argument, naive.core.Formula) and \
                argument.category == naive.core.Formula.ATOMIC_VARIABLE and \
                argument.codomain == b:
            # This argument is a Boolean atomic proposition.
            naive.log.debug('This argument is a Boolean atomic proposition')
            ...
            naive.log.debug(vector=vector)
            argument_vectors[argument_index] = vector
        else:
            naive.log.error('Unexpected type', argument=argument, t=type(argument), category=argument.category,
                      codomain=argument.codomain)
    naive.log.debug(argument_vectors=argument_vectors)
    output_vector = None
    naive.log.debug(phi=phi, arity=phi.arity, system_function=phi.system_function)
    match phi.arity:
        case 0:
        ...
        case _:
            naive.log.error('Arity > 2 are not yet supported, sorry')
    naive.log.debug(output_vector=output_vector)
